...
export class LibraryResolver {
  ...
  @Query(() => [Author])
  async authors(
    @Arg('withoutBooks', { nullable: true }) withoutBooks?: boolean,
  ) {
    if (withoutBooks) {
  ...
  @Query(() => [Book])
  async books(
    @Arg('withoutAuthors', { nullable: true }) withoutAuthors?: boolean,
  ) {
    if (withoutAuthors) {
  ...
  async attachAuthor(
    ...
    if (!book) {
      throw new UserInputError(`Book with id '${bookId}' not found`, {
        bookId,
      });
    }
    ...
    if (!author) {
      throw new UserInputError(`Author with id '${authorId}' not found`, {
        authorId,
      });
    }

    const attachAlreadyExist = book.authors?.some(
      (author) => author.authorId.toString() === authorId.toString(),
    );
